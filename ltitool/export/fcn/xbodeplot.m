...
function [Kast,Kast_ult,wultimo,GM,wcg,PM] = xbodeplot(GH,Kasteric,magud,phaud,delay,w);
...
subplot(2,1,1), grafmag=loglog(w,magud,'Color',myblue,'linewidth',2); hold on
subplot(2,1,1),         loglog([w(ix) w(ix)], [min(magud) magud(ix)],'--','Color',myviolet,'linewidth',1.5, ...
...
subplot(2,1,2), grafpha=semilogx(w,phax,'Color',myblue,'linewidth',2); hold on
subplot(2,1,2),         semilogx([w(ix) w(ix)], [max(phad) phax(ix)],'--','Color',myviolet,'linewidth',1.5,  ...
