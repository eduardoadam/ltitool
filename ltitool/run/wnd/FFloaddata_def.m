...
function btn_closeFFloadata_doIt(source, callbackdata, FFloaddata)
...
Gm = tf(eval(sysStrGm));
% Glt(s)
numGtl = get(dlg.numGtl, 'string');
denGtl = get(dlg.denGtl, 'string');
...
Gt = tf(eval(sysStrGt));
% Cff(s)
numCff = get(dlg.numCff, 'string');
denCff = get(dlg.denCff, 'string');
...
% cierro la ventana
close(dlg.FFloaddata);

end

function PID_rb_doIt(source, callbackdata, FFloaddata)
...
function Compensador_rb_doIt(source, callbackdata, FFloaddata)
...
function ret = show_FFloaddata()
  ...
  Label_31 = uicontrol( ...
	...
	'FontWeight', 'bold', ... 
	'HorizontalAlignment', 'left', ... 
	'Position', [230 609 314 20], ... 
	'String', 'Closed Loop SystemTransfer Functions', ... 
	'TooltipString', '');
  ...
  set (btn_closeFFloadata, 'callback', {@btn_closeFFloadata_doIt, FFloaddata});
  set (PID_rb, 'callback', {@PID_rb_doIt, FFloaddata});
  set (Compensador_rb, 'callback', {@Compensador_rb_doIt, FFloaddata});
  dlg = struct(FFloaddata);
...
% Put data on window
set(FFloaddata.numGp,'string',polyout(Gp.num{1,1})); 
set(FFloaddata.denGp,'string',polyout(Gp.den{1,1}));
set(FFloaddata.numGl,'string',polyout(Gl.num{1,1}));
set(FFloaddata.denGl,'string',polyout(Gl.den{1,1}));
set(FFloaddata.numGm,'string',polyout(Gm.num{1,1}));
set(FFloaddata.denGm,'string',polyout(Gm.den{1,1}));
set(FFloaddata.numGtl,'string',polyout(Gtl.num{1,1}));
set(FFloaddata.denGtl,'string',polyout(Gtl.den{1,1}));
set(FFloaddata.numGt,'string',polyout(Gt.num{1,1}));
set(FFloaddata.denGt,'string',polyout(Gt.den{1,1}));
set(FFloaddata.numGv,'string',polyout(Gv.num{1,1}));
set(FFloaddata.denGv,'string',polyout(Gv.den{1,1}));
set(FFloaddata.numCff,'string',polyout(Cff.num{1,1}));
set(FFloaddata.denCff,'string',polyout(Cff.den{1,1}));
set(FFloaddata.numGcomp,'string',polyout(Gcomp.num{1,1}));
set(FFloaddata.denGcomp,'string',polyout(Gcomp.den{1,1}));


% Controller mode options
if(Kr==0) % P mode option
	set(FFloaddata.Modo_P,'value',0);
	% Without P mode
	set(FFloaddata.Kr,'string','0');
else
	set(FFloaddata.Modo_P,'value',1);
	% if P mode option is true
	set(FFloaddata.Kr,'string',num2str(Kr));
endif
if(TD==0) % D mode option
	set(FFloaddata.Modo_D,'value',0);
	% Without D mode
	set(FFloaddata.TD,'string','0');
else
	set(FFloaddata.Modo_D,'value',1);
	% if D mode option is true
	set(FFloaddata.TD,'string',num2str(TD));
endif
if (TI==0) % I mode option
	set(FFloaddata.Modo_I,'value',0);
	% Without I mode
	set(FFloaddata.TI,'string','0');
else
	set(FFloaddata.Modo_I,'value',1);
	% if I mode option is true 
	set(FFloaddata.TI,'string',num2str(TI));
endif
