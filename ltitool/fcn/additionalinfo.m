...
function [estabilidad,yinf,Test]=additionalinfo(nGol,dGol,Tstop,Nptos);
	delay=0;
	Gol=tf(nGol,dGol);
	[ceros_la,polos_la,kz] = tf2zp(Gol);	% kz es la ganancia asociada a los ceros. It is not the open loop gain.

	% Step response
	DT=Tstop/1000;				% Time interval.
	[resp_escalon,t]=step(Gol,Tstop,DT);

	% Verify the dominant sigma value
